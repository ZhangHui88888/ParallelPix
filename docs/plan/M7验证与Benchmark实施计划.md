...
完成 FR-007～FR-009 的公共基础设施，并以 Sequential 打通 M2→M3→M4→M7→CSV→M1 的真实链路。M5/M6 已通过同一执行器契约接入，未修改 CLI 或 27 列 CSV。
...
| 后端降级 | 未注册后端跳过并返回部分成功 | 已完成 |
| 测试与文档 | 单元、集成、真实进程、M1 Schema 和文档闭环 | 已完成 |
| M5 接入 | 注册 OpenMP 执行器并启用 CPU 跨后端验证 | 代码已完成；待统一测试 |
| M6 接入 | 按运行时注册 CUDA、阶段计时和实际批大小 | 代码已完成；待统一测试 |
| M8 接入 | Hybrid 执行器、CPU 比例和向后兼容的28列 Schema | 见独立 M8 计划 |
...
- M7 不实现 OpenMP/CUDA 像素算法，只注册后端执行器，不写伪造结果。
...
- 混合请求运行所有已注册后端；CUDA 不可用时返回部分成功并生成新的 `run_id`。
- 验证失败行保留原始计时但不发布加速比。
- CUDA 有效行包含稳定的实际批大小和 H2D/Kernel/D2H 中位数。
