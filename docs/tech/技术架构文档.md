...
M7 已负责按实验配置选择后端、计时和包装运行时生命周期。当前注册 Sequential；M5/M6 后续通过 `IBackendExecutor` 接入，不在 M4 中引入统计或 CUDA 同步接口。
...
M3 已作为独立 `parallelpix_io` 静态库加入 CMake。M4 已作为 `parallelpix_processing` 与 `parallelpix_sequential` 静态库实现，覆盖中心裁剪、半像素双线性缩放、亮度、水印混合及整批错误模型。M7 已作为 `parallelpix_benchmark` 静态库实现，真实 Pipeline 完成 M3→M4→PNG→验证→统计→CSV，并在后端未注册时返回可解释的部分成功。`common/` 是后端共享图片模型和像素语义的唯一归属；M5/M6 必须遵守同一规则并通过 M7 执行器接入。
