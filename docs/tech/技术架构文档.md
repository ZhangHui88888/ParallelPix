...
│   └── {common,io,sequential,openmp,cuda,benchmark}/ # M3～M7 接口骨架
├── src/
│   ├── {cli,planning,controller,pipeline}/  # 已实现的 M2
...
M3～M7 当前仅完成目录骨架，不包含可编译实现，也不加入 CMake 目标。`common/` 是后端共享的图片模型和像素处理语义的唯一归属；M4、M5、M6 不应相互复制这些逻辑。M7 只消费后端输出和测量结果，负责验证、统计和 CSV 报告。
