...
```mermaid
flowchart LR
    U["本地 Streamlit 仪表板"] --> X["统一 CLI"]
    X --> A
    A["输入图片与水印"] --> B["Dataset Loader<br/>解码与校验"]
    B --> C["统一 Image Batch"]
    C --> D{"Processing Backend"}
    D --> E["Sequential<br/>单线程 CPU"]
    D --> F["OpenMP<br/>多核 CPU"]
    D --> G["CUDA<br/>GPU"]
...
系统采用“公共流程 + 可替换计算后端”。加载、配置、验证、计时和报告只实现一次，三个后端仅负责相同的像素计算，保证测试条件一致。
...
| 模块 | 职责 | 依赖 |
|------|------|------|
| CLI | 解析后端、路径、线程数和批大小 | C++ |
| Dataset Loader | 扫描、排序、解码和校验图片 | OpenCV |
| Processing Config | 保存三个后端共享的只读参数 | C++ |
| Sequential Backend | 提供单线程基线 | C++ |
| OpenMP Backend | 使用多核 CPU 处理图片或输出行 | OpenMP |
| CUDA Backend | 管理显存、数据传输和 Kernel | CUDA |
| Validator | 比较输出尺寸和像素误差 | C++ |
| Benchmark Runner | 预热、重复测量并计算指标 | C++ |
| Local Dashboard | 启动预定义 Benchmark 命令、展示状态并读取 CSV 图表 | Python、Streamlit、Pandas、Plotly |
...
```text
backend, thread_count, cuda_batch_size, image_count,
input_resolution, output_resolution, compute_ms, end_to_end_ms,
images_per_second, megapixels_per_second, speedup,
parallel_efficiency, validation_passed, max_pixel_error
```

不适用于某后端的字段保持为空，不写伪造的零值。
