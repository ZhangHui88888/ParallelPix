...
M7 已负责按实验配置选择后端、计时和包装运行时生命周期。当前固定注册 Sequential/OpenMP，并在 CUDA 编译器和设备可用时注册 M6；三者均通过 `IBackendExecutor` 接入，不在 M4 中引入统计或 CUDA 同步接口。`BackendAvailability` 区分构建未包含后端与运行时无设备，`BackendExecution` 向 M7 返回实际 CUDA 批大小和可选分阶段时间。
...
M3 已作为独立 `parallelpix_io` 静态库加入 CMake。M4 已作为 `parallelpix_processing` 与 `parallelpix_sequential` 静态库实现，覆盖中心裁剪、半像素双线性缩放、亮度、水印混合及整批错误模型。M5 已作为 `parallelpix_openmp` 静态库实现，按批次规模选择图片级或行级并行。M6 已作为可选 `parallelpix_cuda` 静态库实现，提供运行时探测、二维 Kernel、批量传输、显存复用、OOM 单次降级和阶段计时。M7 已作为 `parallelpix_benchmark` 静态库实现，真实 Pipeline 完成 M3→M4/M5/M6→PNG→验证→统计→CSV，并在 CUDA 未构建或设备不可用时保留 CPU 结果。`common/` 是后端共享图片模型和像素语义的唯一归属。M8 后续只组合 M5/M6，不复制像素算法。
