...
COPY: dict[str, tuple[str, str]] = {
    ...
    "single_image_count_help": (
        "Run one image count per benchmark so the live trajectory has one clear X-axis.",
        "每次基准测试只运行一个图片数量，实时轨迹的 X 轴才保持明确。",
    ),
    ...
    "measurement_cold_start": ("Per-config cold start", "每配置冷启动基准"),
    "cold_start_caption": (
        "No warm-up. Each configuration starts a fresh CLI process and runs once.",
        "无热身；每个配置启动全新的 CLI 进程并仅运行一次。",
    ),
    "measure_cold_start": ("Measure per-config cold starts", "测量每个配置的冷启动"),
    "measure_cold_start_help": (
        "Runs one additional fresh CLI process per configuration. This can take substantially longer.",
        "每个配置会额外启动一个全新的 CLI 进程，运行时间可能明显增加。",
    ),
    "warmup_caption": (
    ...
    "chart_live_trajectory": ("Live processing trajectory", "实时处理轨迹"),
    ...
    "cold_start_not_measured": (
        "Cold-start data is not available for this run. Select Per-config cold start before running to collect it.",
        "本次运行没有冷启动数据。运行前选择“每配置冷启动基准”即可采集。",
    ),
    "median_ms_per_image": ("Median (ms / image)", "中位数（毫秒/张）"),
    "trajectory_not_available": (
        "Run a new benchmark to view its live processing trajectory.",
        "重新运行一次基准测试，即可查看实时处理轨迹。",
    ),
    "images": ("Images", "图片数"),
